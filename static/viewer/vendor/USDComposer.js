	...
	applyTransform( obj, fields, attrs = {} ) {
		...
		if ( xformOpOrder && xformOpOrder.length > 0 ) {
			...
			for ( let i = 0; i < xformOpOrder.length; i ++ ) {
				...
				} else if ( opName === 'xformOp:rotateZYX' ) {

					const r = data[ 'xformOp:rotateZYX' ];
					if ( r ) {

						// USD rotateZYX: matrix = Rz * Ry * Rx.
						// Three.js Euler 'XYZ' order produces the same matrix.
						const euler = new Euler(
							r[ 0 ] * Math.PI / 180,
							r[ 1 ] * Math.PI / 180,
							r[ 2 ] * Math.PI / 180,
